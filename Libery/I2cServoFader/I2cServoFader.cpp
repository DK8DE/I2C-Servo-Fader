...
void I2cServoFader::setLedColor(uint8_t red, uint8_t green, uint8_t blue) {
    setRegister(REGISTER_RED, red);
    setRegister(REGISTER_GREEN, green);
    setRegister(REGISTER_BLUE, blue);
}

void I2cServoFader::setLedBrightness(uint8_t brightness) {
    setRegister(REGISTER_BRIGHTNESS, brightness);
}

void I2cServoFader::setLedState(bool on) {
